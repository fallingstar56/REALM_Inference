...
export GR00T_ROOT="${GR00T_ROOT:-/home/xuhanyuan/Isaac-GR00T}"
...
export GR00T_EMBODIMENT_TAG="${GR00T_EMBODIMENT_TAG:-OXE_DROID_RELATIVE_EEF_RELATIVE_JOINT}"
export GR00T_DEVICE="${GR00T_DEVICE:-cuda:0}"
...
TASK_IDS=(0 2 3 5 6 8 9)
PERTURBATION_IDS=(1 3 4 5 15)

SERVER_PID=""
SERVER_LOG="${SERVER_LOG:-$REALM_ROOT/logs/gr00t_n17_server_$(date +%Y%m%d_%H%M%S).log}"

usage() {
	cat <<EOF
Usage: $(basename "$0") [OPTIONS]

Run REALM with GR00T N1.7 for task IDs: ${TASK_IDS[*]}
and perturbation IDs: ${PERTURBATION_IDS[*]}.

Options:
  --no-uv-sync          Skip 'uv sync --python 3.10' before starting GR00T.
  --reuse-server        Use an existing server on ${HOST}:${PORT}; do not start one.
  --help                Show this help.

Useful environment overrides:
  REALM_ROOT            Default: $REALM_ROOT
  GR00T_ROOT            Default: $GR00T_ROOT
  REALM_DATA_PATH       Default: $REALM_DATA_PATH
  GR00T_MODEL_PATH      Default: $GR00T_MODEL_PATH
  GR00T_DEVICE          Default: $GR00T_DEVICE
...
REUSE_SERVER="${REUSE_GR00T_SERVER:-0}"
...
wait_for_server() {
	...
	echo "Waiting for GR00T server on ${HOST}:${PORT} (timeout: ${MODEL_SERVER_TIMEOUT}s)"
	until port_is_listening; do
		if (( elapsed >= MODEL_SERVER_TIMEOUT )); then
			echo "GR00T server did not become ready within ${MODEL_SERVER_TIMEOUT}s." >&2
	...
	echo "GR00T server is ready on ${HOST}:${PORT}."
}
...
if [[ ! -d "$GR00T_ROOT" ]]; then
	echo "GR00T_ROOT does not exist: $GR00T_ROOT" >&2
	exit 1
fi
...
if [[ ! -f "$GR00T_ROOT/gr00t/eval/run_gr00t_server.py" ]]; then
	echo "GR00T server entrypoint not found under GR00T_ROOT: $GR00T_ROOT" >&2
	exit 1
fi
...
if [[ "$REUSE_SERVER" == "1" ]]; then
	...
	echo "Reusing existing GR00T server on ${HOST}:${PORT}."
elif port_is_listening; then
	echo "A server is already listening on ${HOST}:${PORT}; reusing it."
else
	require_command uv

	if [[ "$SKIP_UV_SYNC" != "1" ]]; then
		echo "Running uv sync for GR00T N1.7 environment."
		(cd "$GR00T_ROOT" && uv sync --python 3.10)
	fi

	echo "Starting GR00T N1.7 server. Log: $SERVER_LOG"
	pushd "$GR00T_ROOT" >/dev/null
...
echo "All GR00T N1.7 REALM evaluations finished."
