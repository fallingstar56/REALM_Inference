...
def _candidate_gr00t_roots() -> list[Path]:
    env_roots = [
...
def _ensure_gr00t_on_path() -> Path | None:
    for candidate in _candidate_gr00t_roots():
...
def _load_policy_client_class():
    try:
        ...
        if exc.name not in {"gr00t", "gr00t.policy", "gr00t.policy.server_client"}:
            raise ImportError(
                "Isaac-GR00T dependencies are missing while importing the official PolicyClient."
            ) from exc

    root = _ensure_gr00t_on_path()
    if root is None:
        raise ImportError(
    ...
    try:
        ...
        raise ImportError(
            f"Failed to import Isaac-GR00T PolicyClient from {root}. "
...
def compute_eef_9d(cartesian_position: Any) -> Any:
    import numpy as np
    ...
    rot_mat = rot_robot @ np.asarray(DROID_EEF_ROTATION_CORRECT, dtype=np.float64)
...
class Gr00tClient:
    def __init__(
        ...
        client_cls = policy_client_cls or _load_policy_client_class()
    ...
    def connect(self, fetch_modality_config: bool = True) -> dict[str, Any] | None:
        if not self.ping():
            raise ConnectionError(f"Failed to reach GR00T server at tcp://{self.host}:{self.port}")
    ...
    def _cache_modality_metadata(self, modality_config: dict[str, Any]) -> None:
        ...
        if not language_keys:
            raise ValueError("GR00T modality config is missing a language key")
        self.language_key = language_keys[0]

        history_window = max(-min(self.video_delta_indices), 0) + 1 if self.video_delta_indices else 1
    ...
    def _build_video_observation(self) -> dict[str, Any]:
        ...
        if not self._frame_buffer:
            raise ValueError("GR00T frame buffer is empty. Call observe() before requesting an action.")
        ...
        *,
        ...
        for state_key in self.state_keys:
            if state_key not in state_sources:
                raise KeyError(f"Unsupported GR00T state key: {state_key}")
    ...
    def _print_observation_stats(self, observation: dict[str, Any]) -> None:
        ...
        for modality_name in ("video", "state"):
            for key, value in observation[modality_name].items():
                print(
                    f"[GR00T observation] {modality_name}.{key}: "
        ...
        for key, value in observation["language"].items():
            ...
            print(
                f"[GR00T observation] language.{key}: "
